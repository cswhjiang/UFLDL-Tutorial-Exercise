...
numFeatures = 100;    % number of features to learn
...
% Initialize matrices
weightMatrix = rand(visibleSize, numFeatures); %A
featureMatrix = rand(numFeatures, batchNumPatches); %s

% Initialize grouping matrix
assert(floor(sqrt(numFeatures)) ^2 == numFeatures, 'numFeatures should be a perfect square');% why
...
%     [cost, grad] = sparseCodingWeightCost(weightMatrix, featureMatrix, visibleSize, numFeatures, patches, gamma, lambda, epsilon, groupMatrix);
    fprintf('  %4d  %10.4f  %10.4f  %10.4f  %10.4f\n', iteration, fResidue+fSparsity+fWeight, fResidue, fSparsity, fWeight)
%     fprintf('  %4d  %10.4f  %10.4f  %10.4f  %10.4f\n', iteration, cost, fResidue, fSparsity, fWeight)
...
%     weightMatrix = zeros(visibleSize, numFeatures);
    ...
    weightMatrix = batchPatches*featureMatrix'*inv(featureMatrix*featureMatrix'+ batchNumPatches*gamma*eye(numFeatures));
...
%     assert(norm(grad) < 1e-12, ['Weight gradient should be close to 0. Check your closed form solution for weightMatrix again. |grad| = ' num2str(norm(grad))]);

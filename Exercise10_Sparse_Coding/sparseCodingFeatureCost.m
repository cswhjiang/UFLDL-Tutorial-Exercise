...
%      grad =  2 * weightMatrix' * T + lambda * groupMatrix * featureMatrix ./ sqrt(groupMatrix * featureMatrix.^2 + epsilon);
    grad =  2 * weightMatrix' * T/numExamples + lambda * groupMatrix'* (1./sqrt(groupMatrix * featureMatrix.^2 + epsilon)) .* featureMatrix;
     grad = grad(:);
end